...
def plot_posterior(thresholds, abc, ax, nb_timepoints):
    ...
    if nb_timepoints == 1:
        ...
        # when multiple timepoints are present, runs can have the idx. In this case,
        # we want to plot runs for which the query is satisfied for all their
        # timepoints: groupby idx (same run with different timepoints), then apply
        # query on those timepoints, then keep run only if all timepoints match
        # query, i.e. shape[0] == timepoints. Finally, take only once the fitness
        # coefficient to avoid saving it multiple times
        to_plot = (
            abc.groupby("idx")
